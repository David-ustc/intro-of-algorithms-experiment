...
void RBTree ::remove(RBTNode *& root, RBTNode * node)
{
    ...
    if ( (node->left!=NULL) && (node->right!=NULL) ) 
    {
        ...
        // "后继结点"肯定不存在非空的左孩子！因为它是一个后继叶子节点。即使有左孩子也是z
        ...
        {
            // 完成transplant(y, y.right)操作 u = u.p.left
            if (child)
                rb_set_parent(child, successor->parent);
    ...
    color = node->color;
    // transplant(z, z.right/left) 和上面一样
    if (node->parent)
    {
        if (node->parent->left == node)
            node->parent->left = child; //用node(z)的子节点代替z
        else
